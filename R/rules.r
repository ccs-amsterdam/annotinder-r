...
rules_crowdcoding <- function(can_seek_backwards = TRUE, units_per_coder = NULL) {
  if (!methods::is(can_seek_backwards, "logical")) stop("can_seek_backwards has to be TRUE or FALSE")
  rules <- list(
    ruleset = jsonlite::unbox("crowdcoding"),
    can_seek_backwards = jsonlite::unbox(can_seek_backwards)
  )
  if (!is.null(units_per_coder)) {
    if (!methods::is(units_per_coder, "numeric")) stop("units_per_coder has to be a number")
...
rules_fixedset <- function(can_seek_backwards = TRUE,
                           can_seek_forwards = FALSE,
                           randomize = FALSE) {
  if (!methods::is(can_seek_backwards, "logical")) stop("can_seek_backwards has to be TRUE or FALSE")
  if (!methods::is(can_seek_forwards, "logical")) stop("can_seek_forwards has to be TRUE or FALSE")

  rules <- list(
    ruleset = jsonlite::unbox("fixedset"),
    can_seek_backwards = jsonlite::unbox(can_seek_backwards),
