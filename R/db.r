...
db_get_codebook <- function(db) {
  if (!DBI::dbExistsTable(db, "codebook")) {
    return(NULL)
  }
...
db_insert_annotation <- function(db, unit_id, annotation) {
  ...
  DBI::dbExecute(db, sprintf(
    "UPDATE annotations SET json=%s WHERE unit_id = '%s'",
    safe_annotation, unit_id
  ))
  DBI::dbExecute(db, sprintf(
    "UPDATE units SET status='%s' WHERE id='%s'",
